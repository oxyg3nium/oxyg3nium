...
import org.oxyg3nium.util.{AVector, Env, Files, TimeStamp}
...
trait BlockFlow
  ...
  private def getSyncLocatorsUnsafe(chainIndex: ChainIndex): AVector[BlockHash] = {
    if (brokerConfig.contains(chainIndex.from)) {
      ...
      if (maxHeight == OXM.GenesisHeight) {
        AVector.empty
      } else {
        val startHeight = Math.max(OXM.GenesisHeight + 1, maxHeight - 2 * maxForkDepth)
        HistoryLocators
  ...
  override protected def getSyncInventoriesUnsafe(
    ...
    locators.mapWithIndex { (locatorsPerChain, index) =>
      ...
      if (locatorsPerChain.isEmpty) {
        chain.getSyncDataFromHeightUnsafe(OXM.GenesisHeight + 1)
      } else {
...
object BlockFlow extends StrictLogging {
  ...
  private def cacheBlockChain(blockflow: BlockFlow, chain: BlockChain)(implicit
    ...
    val startHeight =
      Math.max(OXM.GenesisHeight, maxHeight - consensusSettings.blockCacheCapacityPerChain)
  ...
  private def cacheHeaderChain(chain: BlockHeaderChain)(implicit
    ...
    val maxHeight = chain.maxHeightByWeightUnsafe
    val startHeight =
      Math.max(OXM.GenesisHeight, maxHeight - consensusSettings.blockCacheCapacityPerChain * 2)
    (startHeight to maxHeight).foreach { height =>
