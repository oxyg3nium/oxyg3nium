...
import org.oxyg3nium.protocol.vm.{BlockEnv, WorldState}
import org.oxyg3nium.serde.serialize
import org.oxyg3nium.util.{AVector, Cache, RWLock, TimeStamp}
...
trait MultiChain extends BlockPool with BlockHeaderPool with FlowDifficultyAdjustment {
  ...
  private def getMainChainBlockByGhostUncleUnsafe(
    ...
    val toHeight         = Math.min(ghostUncleHeight + OXM.MaxGhostUncleAge, maxHeight)
