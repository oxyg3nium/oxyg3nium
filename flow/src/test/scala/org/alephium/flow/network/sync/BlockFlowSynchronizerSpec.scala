...
import org.oxyg3nium.flow.handler.{ChainHandler, DependencyHandler, FlowHandler, TestUtils}
...
class BlockFlowSynchronizerSpec extends AlephiumActorSpec {
  ...
  trait Fixture extends FlowFixture with Generators {
    override val configValues: Map[String, Any] = Map(
      ("oxyg3nium.network.enable-p2p-v2", false)
    )
  ...
  trait BlockFlowSynchronizerV2Fixture extends Fixture {
    ...
    override val configValues: Map[String, Any] = Map(
      ...
      ("oxyg3nium.network.enable-p2p-v2", true)
    )
