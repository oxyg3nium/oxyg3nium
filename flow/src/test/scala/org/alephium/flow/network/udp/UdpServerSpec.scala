...
package org.oxyg3nium.flow.network.udp
...
class UdpServerSpec extends Oxyg3niumActorSpec {
  it should "read and write messages" in new Fixture {
    ...
    (0 until 100).foreach { k =>
      val message = ByteString.fromString("oxyg3nium" * k)
  ...
  it should "fail when send data to invalid address" in new Fixture {
    ...
    val message        = ByteString.fromString("oxyg3nium")
