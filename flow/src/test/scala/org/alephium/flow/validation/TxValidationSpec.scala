...
import org.oxyg3nium.protocol.model.{BlockHash => ModelBlockHash, _}
import org.oxyg3nium.protocol.model.ModelGenerators.AssetInputInfo
import org.oxyg3nium.protocol.model.UnsignedTransaction.TxOutputInfo
import org.oxyg3nium.protocol.vm.{InvalidSignature => _, NetworkId => _, _}
...
class TxValidationSpec extends Oxyg3niumFlowSpec with NoIndexModelGeneratorsLike {
  ...
  trait Fixture extends TxValidation.Impl with VMFactory {
    ...
    def prepareOutputs(lockup: LockupScript.Asset, unlock: UnlockScript, outputsNum: Int) = {
      ...
      val outputs =
        AVector.fill(outputsNum + 1)(TxOutputInfo(lockup, OXM.alph(1), AVector.empty, None))
  ...
  it should "check too many inputs" in new Fixture {
    ...
    val modified0 = tx.inputs(AVector.fill(OXM.MaxTxInputNum)(input))
    val modified1 = tx.inputs(AVector.fill(OXM.MaxTxInputNum + 1)(input))
    ...
    val modified3 =
      tx.copy(contractInputs = AVector.fill(OXM.MaxTxInputNum + 1)(contractOutputRef))
  ...
  it should "check too many outputs" in new Fixture {
    ...
    val maxGeneratedOutputsNum = OXM.MaxTxOutputNum - tx.outputsLength

    val modified0 = tx.fixedOutputs(AVector.fill(OXM.MaxTxOutputNum)(output))
    val modified1 = tx.fixedOutputs(AVector.fill(OXM.MaxTxOutputNum + 1)(output))
  ...
  it should "check the number of script signatures" in new Fixture {
    ...
    val modified1 = tx.copy(scriptSignatures = AVector.fill(OXM.MaxScriptSigNum)(signature))
    val modified2 = tx.copy(scriptSignatures = AVector.fill(OXM.MaxScriptSigNum + 1)(signature))
  ...
  it should "check gas bounds deprecated" in new Fixture {
    ...
    tx.gasPrice(GasPrice(OXM.MaxOXMValue - 1)).pass()
    tx.gasPrice(GasPrice(OXM.MaxOXMValue)).fail(InvalidGasPrice)
  }
  ...
  it should "check OXM balance stats" in new Fixture {
  ...
  it should "check the number of public keys in p2mpk for leman fork" in new Fixture {
    forAll(transactionGenWithPreOutputs()) { case (tx, preOutputs) =>
      ...
      val invalidP2MPK = LockupScript.P2MPKH
        .unsafe(AVector(p2pkh.pkHash) ++ AVector.fill(OXM.MaxKeysInP2MPK)(Hash.generate), 1)
    ...
    val keys       = AVector.fill(OXM.MaxKeysInP2MPK)(PublicKey.generate)
  ...
  it should "check the number of public keys in p2mpk for Mainnet fork" in new Fixture {
    val tooManyKeys = AVector.fill(OXM.MaxKeysInP2MPK + 1)(PublicKey.generate)
    val p2mpkh0     = LockupScript.p2mpkh(tooManyKeys.init, 1).value
    val p2mpkh1     = LockupScript.p2mpkh(tooManyKeys, OXM.MaxKeysInP2MPK + 1).value
    val tx0 =
  ...
  it should "check output data size" in new Fixture {
    val oversizedData  = ByteString.fromArrayUnsafe(Array.fill(OXM.MaxOutputDataSize + 1)(0))
    val justEnoughData = ByteString.fromArrayUnsafe(Array.fill(OXM.MaxOutputDataSize)(0))
    oversizedData.length is OXM.MaxOutputDataSize + 1
    justEnoughData.length is OXM.MaxOutputDataSize
