...
import org.oxyg3nium.flow.core.BlockFlowState.{BlockCache, Confirmed}
...
import org.oxyg3nium.protocol.vm.{LockupScript, TokenIssuance}
import org.oxyg3nium.util.{Oxyg3niumSpec, AVector, TimeStamp, U256, UnsecureRandom}

// scalastyle:off file.size.limit
class BlockFlowSpec extends Oxyg3niumSpec {
  ...
  trait InterGroupFixture extends FlowFixture { Test =>
    ...
    val newConfigFixture = new Oxyg3niumConfigFixture {
      override val configValues: Map[String, Any] = Map(
        ("oxyg3nium.broker.broker-id", anotherBroker)
      )
  ...
  it should "not include new block as dependency when dependency gap time is large for pre-rhone hardfork" in new DependencyGapTimeFixture {
    override val configValues =
      Map(
        ("oxyg3nium.consensus.mainnet.uncle-dependency-gap-time", "5 seconds"),
        ("oxyg3nium.network.rhone-hard-fork-timestamp", TimeStamp.Max.millis),
        ("oxyg3nium.broker.broker-num", 1)
      )
    networkConfig.getHardFork(TimeStamp.now()) is HardFork.Leman
  ...
  it should "not include new block as dependency when dependency gap time is large for rhone hardfork" in new DependencyGapTimeFixture {
    override val configValues =
      Map(
        ("oxyg3nium.consensus.rhone.uncle-dependency-gap-time", "5 seconds"),
        ("oxyg3nium.broker.broker-num", 1)
      )
  ...
  it should "include new block as dependency when gap time is past" in new FlowFixture {
    override val configValues =
      Map(
        ("oxyg3nium.consensus.uncle-dependency-gap-time", "5 seconds"),
        ("oxyg3nium.broker.broker-num", 1)
      )
...
trait TxOutputRefIndexFixture extends FlowFixture {
  ...
  override val configValues = Map(
    ("oxyg3nium.broker.broker-num", 1),
    ("oxyg3nium.node.indexes.tx-output-ref-index", s"$enableTxOutputRefIndex"),
    ("oxyg3nium.node.indexes.subcontract-index", "false")
  )
  ...
  def verifyTxIdIndex(lockupScript: LockupScript, block: Block) = {
    ...
    if (enableTxOutputRefIndex) {
      ...
      blockFlow
        ...
        .leftValue
        .reason
        .getMessage is "Please set `oxyg3nium.node.indexes.tx-output-ref-index = true` to query transaction id from transaction output reference"
    }
  }
