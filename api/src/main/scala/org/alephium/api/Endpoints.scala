...
import org.oxyg3nium.api.TapirCodecs
import org.oxyg3nium.api.TapirSchemasLike
import org.oxyg3nium.api.UtilJson.{avectorReadWriter, inetAddressRW}
import org.oxyg3nium.api.model._
import org.oxyg3nium.json.Json.ReadWriter
...
import org.oxyg3nium.protocol.vm.StatefulContract
...
trait Endpoints
  ...
  val buildSweepAddressTransactions
    ...
    transactionsEndpoint.post
      ...
      .summary(
        "Build unsigned transactions to send all unlocked OXM and token balances of one address to another address"
      )
  ...
  val buildSweepMultisig: BaseEndpoint[BuildSweepMultisig, BuildSweepAddressTransactionsResult] =
    multisigEndpoint.post
      ...
      .out(jsonBody[BuildSweepAddressTransactionsResult])
      .summary(
        "Sweep all unlocked OXM and token balances of a multisig address to another address"
      )
