...
trait VotingFixture extends WalletFixture {
  ...
  def buildDeployContractTx(admin: Wallet, voters: Seq[Wallet], tokenAmount: U256): ContractRef = {
    val allocationTransfers = voters.zipWithIndex
      .map { case (_, i) =>
        s"""
           |transferToken!(admin, voters[$i], OXM, $dustAmount)
    ...
    val votingContract =
      s"""
         ...
         |    transferToken!(voter, admin, OXM, $dustAmount)
  ...
  def allocateTokens(
    ...
    val allocationScript =
      s"""
         ...
         |  voting.allocateTokens{caller -> OXM: $dustAmount * ${votersWallets.size}}()
         |}
        $contractCode
      """.stripMargin
    script(
  ...
  def vote(
    ...
    val votingScript =
      s"""
         ...
         |  let caller = callerAddress!()
         |  let voting = Voting(#$contractId)
         |  voting.vote{caller -> OXM: $dustAmount, #$contractId: 1}($choice, caller)
         |}
      $contractCode
      """.stripMargin
    script(
...
trait WalletFixture extends CliqueFixture {
  ...
  val nWallets       = 5
  val walletsBalance = OXM.alph(1000)
