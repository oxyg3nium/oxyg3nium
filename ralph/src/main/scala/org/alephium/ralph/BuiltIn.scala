...
import org.oxyg3nium.protocol.model.dustUtxoAmount
import org.oxyg3nium.protocol.vm._
import org.oxyg3nium.ralph.Compiler.{Error, FuncInfo, UseContractAssetsInfo}
...
object BuiltIn {
  ...
  val payGasFee: SimpleBuiltIn[StatefulContext] =
    SimpleBuiltIn.asset(
      ...
      argsName = Seq(
        ...
        "amount" -> "the amount of gas to be paid in OXM"
      ),
      retComment = "",
      doc = "Pay gas fee."
    )
  ...
  val minimalContractDeposit: SimpleBuiltIn[StatefulContext] =
    SimpleBuiltIn.utils(
      ...
      MinimalContractDeposit,
      argsName = Seq.empty,
      retComment = "the minimal OXM amount for contract deposit",
  ...
  val mapEntryDeposit: SimpleBuiltIn[StatefulContext] =
    SimpleBuiltIn.utils(
      ...
      MinimalContractDeposit,
      argsName = Seq.empty,
      retComment = "the amount of OXM required to create a map entry",
      doc =
        "The amount of OXM required to create a map entry, which is '0.1 OXM' since Rhone upgrade"
    )
