...
class ServerUtilsSpec extends Oxyg3niumSpec {
  ...
  it should "validate unsigned transactions" in new Fixture with TxInputGenerators {
    ...
    ServerUtils.validateUnsignedTransaction(tooMuchGasFee) is Left(
      ApiError.BadRequest(
        "Gas fee exceeds the limit: maximum allowed is 1.0 OXM, but got 20,000.0 OXM. Please lower the gas price or adjust the oxyg3nium.api.gas-fee-cap in your user.conf file."
      )
    )
  ...
  it should "show debug message when contract execution failed" in new Fixture {
    ...
    testError is
      s"> Contract @ ${Address.contract(testContract.contractId).toBase58} - Hello, Oxyg3nium!\n" ++
  ...
  it should "extract debug message from contract event" in new Fixture {
    ...
    serverUtils
      .extractDebugMessage(buildEvent(ValByteVec(ByteString.fromString("Hello, Oxyg3nium!")))) isE
      DebugMessage(contractAddress, "Hello, Oxyg3nium!")

    serverUtils
      .extractDebugMessage(
        buildEvent(ValByteVec(ByteString.fromString("Hello, Oxyg3nium!")), ValBool(true))
      )
      .leftValue
      .detail is "Invalid debug message"
  }
  ...
  it should "test debug function for Ralph" in new Fixture {
    ...
    val testContract = TestContract(
      bytecode = code,
      initialImmFields = Some(AVector(ValByteVec(ByteString.fromString("Oxyg3nium"))))
    ).toComplete().rightValue
    val serverUtils = new ServerUtils()
